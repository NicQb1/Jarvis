...
namespace Network
{
    public class Utilities
    {
        ...
        public int spiralInterval = 100;
        public int levelLimit = 3;
        public int levelCount = 0;
        List<PointF> AllPoints = new List<PointF>();


        private List<PointF> getInputPoints(int x, int y, int spInt)
        {
            ...
            while (radius > 2)
            {
                ...
                try
                {
                    ...
                    if (count == spiralInterval)
                    {
                        ...
                        {
                            drawNewSpiral(e, new_point, startPoint, theta + (dtheta), 1, true);
                        }
                        count = 0;
                    }
                    theta += dtheta;
            ...
            while (radius > 2)
            {
                ...
                try
                {
                    ...
                    if (count == spiralInterval)
                    {
                        ...
                        if (Math.Abs(startPoint.X - new_point.X) > 2 && Math.Abs(startPoint.Y - new_point.Y) > 2)
                        {
                            drawNewSpiral(e, new_point, startPoint, theta + (dtheta), 1, false);
                        }
                        count = 0;
                    }
                    theta += dtheta;
        ...
        public void drawNewSpiral(PointF start, PointF origin, double theta, int level, bool reverse )
        {
            if (level >= levelLimit)
            {
                return;
            ...
            while (radius > 2)
            {
                ...
                try
                {
                    ...
                    if (count >= (spiralInterval / level))
                    {
                        ...
                        if (Math.Abs(startPoint.X - new_point.X) > 2 && Math.Abs(startPoint.Y - new_point.Y) > 2)
                        {
                            drawNewSpiral(e, new_point, startPoint, theta + (dtheta), level + 1, !reverse);
                        }
                        count = 0;
                    }
                    if (reverse)
